...
<%
	...
	try {
		...
		ResultSet rs = st.executeQuery(
				"select * from login where password='" + currentPassword + "' AND SID='" + id + "'");
		while (rs.next()) {
			ids = rs.getInt(1);
			pass = rs.getString(3);
		}
		...
		if (pass.equals(currentPassword)) {
			...
			int i = st1.executeUpdate("update login set password='" + Newpass + "' where SID='" + id + "'");
			...
			session.setAttribute("fail", "Invalid Current Password! Please try again.");
	        response.sendRedirect("Login.jsp");
		}
	} catch (Exception e) {
		out.println(e);
